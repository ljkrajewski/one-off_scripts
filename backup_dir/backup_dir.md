...
## OPTIONS
- **src**  
  Source directory to back up (required).

- **dest**  
  Destination directory (required, must exist).

- **--type** {*full*|*incr*}  
  Backup type: *full* (default) or *incr* (incremental).

- **--timestamp** *timestamp_file*  
  Path to timestamp file. Required for *incr* mode; optional for *full* mode. Updated to backup start time after successful backup.

- **--log** *log_file* | **-l** *log_file*  
  Write logs to *log_file* instead of stdout.

- **--verbose** | **-v**  
  Enable detailed logging, including all file operations and progress.
